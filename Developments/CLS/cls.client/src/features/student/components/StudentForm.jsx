...
export const StudentForm = ({ student, onSubmit, isLoading, onCancel }) => {
  ...
  const {
    ...
    defaultValues: isEdit
      ? { fullName: student.fullName, dateOfBirth: student.dateOfBirth ?? '' }
      : {},
  });

  useEffect(() => {
    if (student) {
      reset({ fullName: student.fullName, dateOfBirth: student.dateOfBirth ?? '' });
    }
  }, [student, reset]);
  ...
  return (
    <form onSubmit={handleSubmit(onSubmit)} className="space-y-5">
      ...
      {/* ── Thông tin phụ huynh (chỉ khi tạo mới) ─────────────────────── */}
      {!isEdit && (
        <div>
          <h3 className="text-sm font-semibold text-gray-500 uppercase tracking-wider mb-3">
            Thông tin phụ huynh
