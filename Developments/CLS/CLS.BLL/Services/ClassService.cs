...
public class ClassService : IClassService
{
    ...
    {
        var classEntity = await _classRepo.GetByIdWithDetailsAsync(id, ct);
        ...
        CreateClassRequest request, int createdBy, CancellationToken ct = default)
    {
        ...
        var created = await _classRepo.GetByIdWithDetailsAsync(entity.Id, ct);
        ...
        int id, UpdateClassRequest request, CancellationToken ct = default)
    {
        ...
        var updated = await _classRepo.GetByIdWithDetailsAsync(id, ct);
    ...
    {
        var entity = await _classRepo.GetByIdWithDetailsAsync(classId, ct);
        ...
        int classId, EnrollStudentsRequest request, CancellationToken ct = default)
    {
        ...
        foreach (var studentId in distinctIds)
        {
            ...
            if (student.Status != AppConstants.StudentStatus.Active)
            {
                errors.Add($"Học sinh '{student.FullName}' không ở trạng thái hoạt động.");
                continue;
            }
        ...
        var reloaded = await _classRepo.GetByIdWithDetailsAsync(classId, ct);
        var result = reloaded!.ClassStudents
            .Where(cs => enrolled.Select(e => e.StudentId).Contains(cs.StudentId))
            .OrderBy(cs => cs.Student.FullName);
        ...
        int classId, int studentId, CancellationToken ct = default)
    {
        ...
        var enrollment = classEntity.ClassStudents
            .FirstOrDefault(cs => cs.StudentId == studentId && cs.Status == "active");
