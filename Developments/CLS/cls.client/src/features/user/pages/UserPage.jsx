...
export default function UserPage() {
  ...
  const handleCreate = (data) => { createUser.mutate(data, { onSuccess: () => { setShowForm(false); createUser.reset(); toast.success('Tạo tài khoản thành công!'); }, onError: (e) => toast.error(e.message || 'Lỗi.') }); };
  const handleUpdate = (data) => { if (!editingUser) return; updateUser.mutate({ id: editingUser.id, ...data }, { onSuccess: () => { setEditingUser(null); setShowForm(false); updateUser.reset(); toast.success('Cập nhật tài khoản thành công!'); }, onError: (e) => toast.error(e.message || 'Lỗi.') }); };
  const handleEdit = (user) => { setEditingUser(user); setShowForm(true); };
  const handleToggleStatus = (user) => setConfirmingUser(user);

  const handleResetPassword = (user) => {
    resetPw.mutate(user.id, {
      onSuccess: (data) => { setResetResult({ userName: user.fullName, newPassword: data.newPassword }); },
      onError: (e) => toast.error(e.message || 'Không thể đặt lại mật khẩu.'),
    });
  };
  ...
  const confirmToggle = () => {
    ...
    updateStatus.mutate({ id: confirmingUser.id, status: newStatus }, { onSuccess: () => { toast.success(`Đã ${label} tài khoản "${confirmingUser.fullName}".`); setConfirmingUser(null); }, onError: (e) => { toast.error(e.message); setConfirmingUser(null); } });
  };
  ...
  return (
    <div className="p-6 max-w-7xl mx-auto">
      <div className="flex items-center justify-between mb-6">
        <div><h1 className="text-2xl font-bold text-gray-800">Quản lý tài khoản</h1><p className="text-sm text-gray-500 mt-1">Tạo và quản lý tài khoản giáo viên</p></div>
        <button onClick={() => { setEditingUser(null); setShowForm(true); }} className="px-4 py-2.5 bg-indigo-600 text-white text-sm font-medium rounded-lg hover:bg-indigo-700 transition-colors flex items-center gap-2 shadow-sm">
          <svg className="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M12 4v16m8-8H4" /></svg>Tạo tài khoản
        </button>
      </div>
      ...
      {confirmingUser && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40">
          <div className="bg-white rounded-2xl shadow-2xl p-6 w-full max-w-sm mx-4">
            ...
            <p className="text-sm text-gray-500 text-center mt-2">Bạn có chắc muốn <span className="font-medium text-gray-700">{confirmLabel}</span> tài khoản <span className="font-semibold text-gray-900">&quot;{confirmingUser.fullName}&quot;</span>?</p>
      ...
      {resetResult && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40">
          <div className="bg-white rounded-2xl shadow-2xl p-6 w-full max-w-sm mx-4">
            <div className="w-12 h-12 mx-auto mb-4 rounded-full bg-green-100 flex items-center justify-center"><span className="text-2xl">🔑</span></div>
            <h3 className="text-lg font-semibold text-gray-900 text-center">Mật khẩu mới</h3>
            <p className="text-sm text-gray-500 text-center mt-2">Mật khẩu mới của <span className="font-semibold">{resetResult.userName}</span>:</p>
            <div className="mt-3 bg-gray-100 rounded-lg p-3 text-center font-mono text-lg text-gray-900 select-all">{resetResult.newPassword}</div>
